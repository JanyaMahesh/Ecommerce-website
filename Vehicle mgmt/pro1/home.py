...
def employee():
    if request.method == 'POST':
        try:
            ...
            return render_template("wrong_emp.html")
...
def owner():
    if request.method == 'POST':
        try:
            ...
            return render_template("wrong_owner.html")
    return render_template("password.html")
...
def agent():
    if request.method == 'POST':
        try:
            ...
            return render_template("wrong_agent.html")
    return render_template("password.html")
...
def query4():
        cur.execute("select substr(mfg_id,1,2) as company,count(*) from sales group by substr(mfg_id,1,2)")
        rows = cur.fetchall()
        col = [desc[0] for desc in cur.description]
        return render_template('view.html',content = [rows,col])
